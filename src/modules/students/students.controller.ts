...
export class StudentsController {
  ...
  async getStudentList(@Query() studentPageQuery: StudentPageQuery) {
    const { totalCount, students } =
      await this.studentsService.getStudentList(studentPageQuery);
    const contents = (await students).map((student) => new StudentDto(student));
    const pageDto = new PageDto(
      studentPageQuery.pageNumber,
      studentPageQuery.pageSize,
      totalCount,
      contents,
    );
  ...
  async getStudentExcel(
    @Query() studentExcelQuery: GetStudentExcelQuery,
    @Response() res,
  ) {
    const { fileName, stream } =
      await this.studentsService.getStudentExcel(studentExcelQuery);
